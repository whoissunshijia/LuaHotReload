...
function HU.InitFileMap(RootPath)
	local TheMap = {}
	local scriptPath=UE4.UFixLuaRuntime.GetScriptPath()
	local l=UE4.UFixLuaRuntime.GetAllFile(scriptPath)
	for i=1,l,1 do
		local FixLuaName=(UE4.UFixLuaRuntime.GetFixLuaName(i-1))
		local FixPath=(UE4.UFixLuaRuntime.GetFixPath(i-1))
		local FixLuaPathName=(UE4.UFixLuaRuntime.GetFixLuaPathName(i-1))
		local FileName=FixLuaName
		if FileName ~= nil then
	        if TheMap[FileName] == nil then
	            TheMap[FileName] = {}
	        end
	    end
		table.insert(TheMap[FixLuaName], {SysPath = FixPath, LuaPath = FixLuaPathName})
	end
	return TheMap
end
...
function HU.LoadStringFunc(SysPath)
	local CodeStr = UE4.FFileHelper.LoadFileToArray(SysPath)
	return CodeStr
end
...
function HU.UpdateAllFunction(OldTable, NewTable, Name, From, Deepth)
	...
	for ElementName, Element in pairs(NewTable) do
		...
		if type(Element) == type(OldElement) then
			if type(Element) == "function" then
				...
				rawset(package.loaded[Name],ElementName,Element)
				TableFunction[ElementName]=Element
	...
	if HU.HUMap[Name] ~= nil then
		HU.NewFunctionTable[Name]=TableFunction
	end
