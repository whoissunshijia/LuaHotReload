...
bool UUnLuaManager::OnModuleHotfixed(const TCHAR *InModuleName)
{
	...
	if (ClassPtr)
	{
		...
		//获取 需要覆写的反射UFunction
		//之前 如果有的话 不会覆写之前的,但是可以修改逻辑
		//如果不是UFunction不会覆写
		TSet<FName> NoNativeFunction;
		TSet<FName> ConvertToUseNativeFunction;
		TSet<FName> *Functiones = NewUpdateLuaFunction.Find(InModuleName);
		if (!Functiones)
			return false;
		TSet<FName> FunctionesNeedToUpdate = LuaFunctions.Intersect(*Functiones);
		if (FunctionesNeedToUpdate.Num() > 0)
		{
			...
			for (const FName &LuaFuncName : FunctionesNeedToUpdate)
			{
				...
				// 不在UFunction之列的函数,从lua Module删除,只保留可以覆写的
				else
				{
					NoNativeFunction.Add(LuaFuncName);
				}
			}
			bSuccess = ConditionalUpdateClass(Class, FunctionesNeedToUpdate, *UEFunctionsPtr);        // update class conditionally
		}

		//lua文件 没有更新的反射UFunction在这里处理为还是调用c++或蓝图的.
		ConvertToUseNativeFunction = LuaFunctions.Difference(FunctionesNeedToUpdate);
		if (ConvertToUseNativeFunction.Num() > 0)
		{
			UClass *Class = *ClassPtr;
			TMap<FName, UFunction*> *UEFunctionsPtr = OverridableFunctions.Find(Class);
			for (const FName &LuaFuncName : ConvertToUseNativeFunction)
			{
				UFunction **Func = UEFunctionsPtr->Find(LuaFuncName);
				if (Func)
				{
					UFunction* i = GReflectionRegistry.FindOverriddenFunction(*Func);
					//在蓝图内覆写了c++声明的函数,或者c++内没有覆写 只在蓝图内定义的函数
					if (i)
					{
...
void UUnLuaManager::AddNewUpdateLuaFunctiones(const TCHAR* ModuleName, const TCHAR* FunctionName)
{
	TSet<FName>* Funes = NewUpdateLuaFunction.Find(ModuleName);
	if (!Funes)
	{
		Funes = &NewUpdateLuaFunction.Add(ModuleName);
	}
	Funes->Add(FunctionName);
}
