...
bool AddNewUpdateLuaFunctiones(const char* ModuleName, const char* FunctionName)
{
	if (!SManager || !ModuleName)
	{
		UE_LOG(LogUnLua, Log, TEXT("%s: Invalid module name!"), ANSI_TO_TCHAR(__FUNCTION__));
	...
	SManager->AddNewUpdateLuaFunctiones(ANSI_TO_TCHAR(ModuleName), ANSI_TO_TCHAR(FunctionName));
	return true;
}

EXPORT_FUNCTION(bool, OnModuleHotfixed, const char*)
EXPORT_FUNCTION(bool, AddNewUpdateLuaFunctiones, const char*, const char*)
